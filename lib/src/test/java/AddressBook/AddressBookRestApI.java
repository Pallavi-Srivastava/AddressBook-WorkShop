...
public class AddressBookRestApI {
	...
	@Test
	public void givenExistigEmployeeToDelete_WhenDeleted_ShouldMatch200ResponseAndCount() {
		PersonData[] gsonContacts = getEmployeeList();
		addressBook = new AddressBookService(Arrays.asList(gsonContacts));
		PersonData addressBookData = addressBook.getAddressBookData("pallavi");
		RequestSpecification request = RestAssured.given();
		request.header("Content-Type", "application/json");
		Response response = request.delete("/address/" + addressBookData.firstName);
		int statusCode = response.getStatusCode();
		assertEquals(200, statusCode);
		addressBook.deleteAddressBook(addressBookData.firstName, IOService.REST_IO);
		long entries = addressBook.countEnteries(IOService.REST_IO);
